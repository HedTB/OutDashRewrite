...
class LoggingEvents(commands.Cog):
    ...
    async def on_bulk_message_delete(self, messages: list[disnake.Message]):
        try:
            webhook = LoggingWebhook(
                self.bot.user.avatar, messages[0].guild, "message_bulk_delete"
            )
    ...
    async def on_message_edit(self, before: disnake.Message, after: disnake.Message):
        ...
        try:
            webhook = LoggingWebhook(
                self.bot.user.avatar, before.guild, "message_bulk_delete"
            )
        except InvalidWebhook:
            return
        ...
        after_text = (
            after.content if len(after.content) < 1021 else f"{after.content[:1021]}..."
        )
        ...
        if message_content_privacy != "true":
            ...
            embed.add_field(
                name="Notice", value="`This user has message content privacy enabled.`"
            )
    ...
    async def on_message_delete(self, message: disnake.Message):
        ...
        try:
            webhook = LoggingWebhook(
                self.bot.user.avatar, message.guild, "message_delete"
            )
        except InvalidWebhook:
            return
    ...
    async def on_member_remove(self, member: disnake.Member):
        try:
            webhook = LoggingWebhook(
                self.bot.user.avatar, member.guild, "member_remove"
            )
        except InvalidWebhook:
            return
        ...
        embed.add_field(
            name="Member Since",
            value=f"{functions.seconds_to_text(joined_seconds_ago, 3)} ago",
            inline=False,
        )
    ...
    async def on_member_update(self, before: disnake.Member, after: disnake.Member):
        try:
            webhook = LoggingWebhook(
                self.bot.user.avatar, before.guild, "member_update"
            )
        except InvalidWebhook:
            return
        ...
        if before.nick != after.nick:
            embed = disnake.Embed(
                description="Nickname changed", color=colors.logs_embed_color
            )
            ...
            embed.add_field(
                "After",
                disnake.utils.escape_markdown(after.nick if after.nick else after.name),
            )
