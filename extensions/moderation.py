...
class Moderation(commands.Cog):
    ...
    async def warn(self, ctx: commands.Context, member: disnake.Member, *, reason: str):
        ...
        if member == ctx.author:
            ...
            functions.is_role_above_role(member.top_role, ctx.author.top_role)
        ...
        dm_embed = disnake.Embed(
            description=f"{moderator} You have been warned in **{ctx.guild.name}**.\n**Reason**: {reason}",
    ...
    async def warn_remove(
        ...
        if not result:
            ...
            embed = disnake.Embed(
                description=f"{yes} The warning with ID {warning_id} has been removed.",
                color=colors.success_embed_color,
            )
    ...
    async def warnings(self, ctx: commands.Context, member: disnake.Member = None):
        ...
        if len(warnings) == 0:
            ...
            for warning in warnings:
                ...
                description += (
                    f"**#{warning_num} | Moderator: {self.bot.get_user(moderator)}**\n"
    ...
    @is_moderator(ban_members=True)
    async def unban(
        self, ctx: commands.Context, user: disnake.User, *, reason="No reason provided."
    ):
    ...
    async def kick(
        ...
        *,
        ...
        if member == ctx.author:
            ...
            embed = disnake.Embed(
                description=f"{no} I can't kick myself!", color=colors.error_embed_color
            )
            return await ctx.send(embed=embed)
    ...
    async def mute(
        ...
        *,
        ...
        if member == ctx.author:
            ...
            embed = disnake.Embed(
                description=f"{no} I can't mute myself!", color=colors.error_embed_color
            )
            return await ctx.send(embed=embed)
    ...
    async def slowmode(
        self, ctx: commands.Context, seconds: int, channel: disnake.TextChannel = None
    ):
        ...
        if channel == ctx.channel:
            ...
            embed = disnake.Embed(
                description=f"{yes} The slowmode for {channel.mention} has been set to `{seconds}` seconds.",
                color=colors.success_embed_color,
            )
    ...
    async def clear(
        self, ctx: commands.Context, amount: int, channel: disnake.TextChannel = None
    ):
        ...
        if not channel or channel and channel == ctx.channel:
            ...
            embed.description += (
                ...
                if not deleted == 1
                else f"{yes} Deleted {deleted} messsage."
            )
            await ctx.send(embed=embed)
            ...
            embed.description += (
                ...
                if deleted != 1
                else f"{yes} Deleted {deleted} messsage in <#{channel.id}>."
            )
    ...
    @is_moderator(manage_guild=True)
    async def slash_warnings_clear(
        self,
        inter: disnake.ApplicationCommandInteraction,
        member: disnake.Member,
        reason: str = "No reason provided.",
    ...
    async def slash_unban(
        ...
        *,
        ...
        dm_fail_embed = disnake.Embed(
            description=f"{yes} **{user}** was unbanned. I couldn't DM them though.",
            color=colors.success_embed_color,
        )
        dm_embed = disnake.Embed(
            description=f"{moderator} You have been unbanned from **{inter.guild.name}**.",
            color=colors.embed_color,
        )
    ...
    @commands.slash_command(
        name="moderators", description="View and manage the moderators of the server."
    )
    @is_moderator(administator=True)
    async def slash_moderators(self, inter: disnake.ApplicationCommandInteraction):
        """View and manage the moderators of the server."""
        pass

    @slash_moderators.sub_command(
        name="add", description="Add a moderator to the server."
    )
    @is_moderator(administator=True)
    async def slash_moderators_add(
